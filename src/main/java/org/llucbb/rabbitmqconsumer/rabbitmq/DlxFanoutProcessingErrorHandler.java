 ...
 * though (for example if you has Employee JSON structure to process, but got Animal JSON structure instead from Rabbit
 * MQ queue).
 * </p>
 *
 * <p>
 * In short, this is just a class to avoid boilerplate codes for your handler. Default implementation is re-throw
 * message to dead letter exchange, using
...
public class DlxFanoutProcessingErrorHandler {
     ...
     * put it on dead exchange with given <code>dlxExchangeName</code> and <code>routingKey</code>
     *
     * @param deadExchangeName dead exchange name. Not a dlx for work queue, but exchange name for really dead message
     *                         (wont processed antmore).
     * @param maxRetryCount    number of retry before message considered as dead (0 >= <code> maxRetryCount</code> >=
     *                         1000). If set less than 0, will always retry
     * @throws IllegalArgumentException if <code>dlxExchangeName</code> or <code>dlxRoutingKey</code> is null or empty.
     */
    ...
    /**
     * Handle AMQP message consume error. This default implementation will put message to dead letter exchange for
     * <code>maxRetryCount</code> times, thus two variables are required when creating this object:
     * <code>dlxExchangeName</code> and <code>dlxRoutingKey</code>. <br/>
     * <code>maxRetryCount</code> is 3 by default, but you can set it using <code>setMaxRetryCount(int)</code>
     *
